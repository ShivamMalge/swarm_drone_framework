...
\begin{abstract}
Decentralized swarm autonomy requires agents to coordinate under unreliable communication, partial observability, and irreversible energy depletion. This paper presents a modular, event-driven simulation framework for decentralized drone swarms that enforces strict fog-of-war constraints: agents operate exclusively on stale neighbor beliefs maintained in local maps, with no access to global topology. The simulation kernel implements a deterministic discrete-event model with independent random number streams, enabling exact replay. Inter-agent communication is modeled as a Random Geometric Graph in which each transmission survives with a probability jointly determined by a baseline drop rate, an exogenous interference field, and distance-dependent path loss. Three distributed coordination algorithms---gossip-based consensus, localized Voronoi coverage, and auction-based task allocation---execute solely on local information. A passive regime detection layer classifies the environment into operational states, driving a hybrid supervisor whose proposed parameter adjustments pass through a static safety box constraint, a bisection search against a locally computed delay-tolerant stability bound, and exponential moving average smoothing. Across 50 Monte Carlo runs at 100 agents, the framework reduces the mean energy decay rate by $4.5\times$ relative to an unconstrained baseline ($0.050 \pm 0.001$ against $0.228 \pm 0.018$ units/tick), with mean per-update parameter shifts of $0.016$. We report two negative results of equal weight. First, the framework does not delay the loss of half the swarm: time to 50\% attrition is $101 \pm 2$ ticks against the baseline's $100 \pm 2$. Second, a centralized oracle holding exact global state improves swarm half-life roughly twofold when charged neighborhood-local communication costs, yet is the shortest-lived configuration studied once billed for the all-to-all transmission that global awareness implies---under this energy model, the bandwidth cost of centralization consumes its entire coordination benefit. We trace the energy result to a stationary fixed point of the localized coverage law under isolation, rather than to the stability projector to which it was initially attributed.\footnote{The complete simulation kernel, baseline configurations, and execution scripts are open-source: \texttt{https://github.com/ShivamMalge/swarm\_drone\_framework}}
\end{abstract}
...
To ensure statistical significance and validate the robustness of the architecture, all quantitative experiments were averaged over 50 independent Monte Carlo runs utilizing randomized initial PRNG seeds. We report the mean metrics alongside 95\% confidence intervals. Table \ref{tab:baselines} provides a quantitative ablation against an Unconstrained RGG baseline (which lacks our safety bounding) and a Centralized Oracle that receives exact global state every tick. We deliberately do not treat the oracle as a performance upper bound. Because this work studies coordination under \emph{joint} communication and energy constraints, global awareness must be paid for: the oracle is billed for transmission to every surviving agent, whereas decentralized agents pay only for neighbours inside $R_{tx}$. Under that accounting the oracle is charged for approximately $99$ recipients per broadcast against roughly $9$ for the decentralized arms, a $10.4\times$ per-transmission penalty, and it does not outlive the proposed framework. We report this as a result rather than an artifact, and quantify the accounting's contribution separately in Table \ref{tab:oracle_sensitivity}.

\begin{table}[htbp]
\caption{Quantitative Baseline Comparison (50 Monte Carlo Runs, 95\% CI). Time to 50\% attrition is the primary measure: it is uncensored, whereas total swarm survival is right-censored at the $2000$-tick simulation horizon.}
\begin{center}
\begin{tabularx}{\linewidth}{@{}l*{3}{>{\centering\arraybackslash}X}@{}}
\toprule
\textbf{Methodology} & \textbf{Time to 50\% Attrition (ticks)} & \textbf{Energy Decay Rate (units/tick)} & \textbf{Total Swarm Survival (ticks)} \\
\midrule
Unconstrained RGG & $100 \pm 2$ & $0.228 \pm 0.018$ & $476 \pm 38$ \\
Centralized Oracle (global info, all-to-all comm. cost) & $21 \pm 0$ & $2.801 \pm 0.207$ & $39 \pm 4$ \\
\textbf{Proposed Framework} & $\mathbf{101 \pm 2}$ & $\mathbf{0.050 \pm 0.001}$ & $> 2000$ (49/50 censored) \\
\bottomrule
\end{tabularx}
\label{tab:baselines}
\end{center}
...
\begin{table}[htbp]
\caption{Oracle Communication-Accounting Sensitivity (10 Monte Carlo Runs, 95\% CI). Survival is right-censored at $t=2000$; ``cens.'' reports how many runs never experienced total swarm death, for which the mean is a lower bound.}
\begin{center}
\begin{tabularx}{\linewidth}{@{}l*{4}{>{\centering\arraybackslash}X}@{}}
\toprule
\textbf{Arm} & \textbf{Decay Rate (units/tick)} & \textbf{Survival (ticks)} & \textbf{Cens.} & \textbf{Time to 50\% Attrition (ticks)} \\
\midrule
Unconstrained & $0.257 \pm 0.044$ & $427 \pm 88$ & 0/10 & $102 \pm 4$ \\
\textbf{Proposed} & $\mathbf{0.050 \pm 0.000}$ & $\mathbf{2000 \pm 0}$ & \textbf{10/10} & $\mathbf{102 \pm 4}$ \\
Oracle, all-to-all cost & $2.576 \pm 0.477$ & $43 \pm 10$ & 0/10 & $21 \pm 0$ \\
Oracle, per-neighbour cost & $0.070 \pm 0.007$ & $1454 \pm 137$ & 0/10 & $199 \pm 5$ \\
\bottomrule
\end{tabularx}
\label{tab:oracle_sensitivity}
\end{center}
\end{table}

Table \ref{tab:oracle_sensitivity} separates the oracle's coordination quality from its communication cost. Billed all-to-all, the oracle is the shortest-lived arm in the study: perfect global awareness is bought at a bandwidth price this energy model cannot sustain, and centralization does not pay for itself. Billed per-neighbour --- that is, granted global information for free and charged exactly as the decentralized arms are --- the oracle roughly doubles the time to 50\% attrition ($199$ versus $102$ ticks) and is the only arm that meaningfully improves that metric. It nonetheless records lower total-swarm survival than the Proposed framework ($1454$ versus a censored $2000$), because total-swarm survival is determined by a small number of stationary stragglers rather than by coordination quality; we therefore treat time to 50\% attrition as the discriminating measure and report total survival only with its censoring made explicit. The decomposition is the substantive point: centralized coordination is worth roughly a doubling of swarm half-life, and under this energy model that benefit is entirely consumed by the cost of the communication required to obtain it.

Table \ref{tab:baselines} supports one substantive claim for the proposed framework and refutes another that an earlier revision of this work asserted.

The supported claim is thermodynamic. The proposed framework reduces the mean energy decay rate to $0.050 \pm 0.001$ units/tick against $0.228 \pm 0.018$ for the unconstrained baseline, a $4.5\times$ reduction with well separated confidence intervals, and it is the only arm whose swarm had not fully expired at the $2000$-tick horizon.

The refuted claim concerns attrition. The framework does \emph{not} delay the loss of half the swarm: time to 50\% attrition is $101 \pm 2$ ticks against the unconstrained baseline's $100 \pm 2$, a difference well inside the confidence intervals. Whatever the safety projector contributes, it does not manifest before half the agents are already lost; the benefit is confined to the surviving remnant, which persists far longer at a much lower drain. We report total swarm survival as a censored lower bound rather than a point estimate for the same reason: $49$ of $50$ proposed-framework runs reached the horizon without total swarm death, so a mean over those runs measures the simulation cap and the behaviour of a small number of near-stationary survivors, not swarm lifetime.

The Proposed Framework does not achieve this by possessing global information. Tracing the executed control path shows that the difference arises from which velocity law each condition ends up running once the network fragments, and we report that mechanism directly rather than attributing the result to a stability property it does not exhibit.
...
The framework is designed to maintain connectivity under severe interference. Figure \ref{fig:spectral} demonstrates that as environmental jamming increases to $100\%$, a standard non-adaptive RGG framework experiences rapid topological fragmentation. Conversely, the proposed architecture's Hybrid Supervisor autonomously triggers localized spatial clustering. However, the empirical local mixing proxy of Eq. (2) is not a calibrated estimator of the global Fiedler value. In a single $N=200$ jamming run (Figure \ref{fig:spectral}), the proxy exceeds the true $\lambda_2$ by a mean of $+1.81$, against a true $\lambda_2$ that never exceeds $0.151$ over the whole run: the two quantities differ by more than an order of magnitude in scale, so the discrepancy is one of calibration rather than a bounded bias. The direction of the error follows from what we term the ``Disconnected Subgraph Paradox.'' If a subset of agents completely fragments from the main swarm, their local consensus variance drops to zero; the software then applies an explicit guard against the resulting division by zero and returns a maximal proxy value, so total isolation is reported as ideal mixing. The architecture attempts to mitigate this bias by requiring a triad of metrics (variance decay, neighbor density, and staleness) to validate connectivity health. We make no claim that this stabilizes the network: under corrected measurement, in which the algebraic connectivity is computed over living agents only, $\lambda_2$ does not distinguish the proposed framework from the unconstrained baseline in the one window where their attrition rates match ($0.195 \pm 0.032$ against $0.184 \pm 0.034$ for $t \le 100$ ticks). Cross-arm comparison outside that window is confounded by differential survival and by swarm size, since a graph over fewer surviving agents has lower $\lambda_2$ irrespective of coordination quality. We therefore report connectivity as a diagnostic and do not treat it as a discriminating outcome.
