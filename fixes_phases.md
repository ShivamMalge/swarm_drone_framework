...
✅ **b19/b20/b21 RESOLVED (web + local-PDF verification, 2026-08-18):** all three turned out to be IN the repo under opaque filenames. b19 = `07526474.pdf` (its IEEE Xplore ID) — ACC 2016, pp. 5136–5140, entry was correct. b20 = `[J186].pdf` — L-CSS vol. 9, 2025, but real pages are **1640–1645**, not the "100–105" printed; fixed. b21 = `2412.02146v2.pdf` — the arXiv abstract page lists **no journal-ref**: the "IEEE TAC vol. 69(5), pp. 2000–2010, 2024" metadata was **fabricated** (the arXiv header is a submission template). Recited as an arXiv preprint.

**Verified additions (author-requested fetch, each attached to an existing claim — no padding):** [25] Preiss et al., Crazyswarm, ICRA 2017 (Crazyswarm was named uncited after the SwarmFly fix); [26] Kaul/Yates/Gruteser, INFOCOM 2012, pp. 2731–2735 (Age of Information — anchors §III-D's belief-age eviction horizon); [27] Kaplan & Meier, JASA 53(282):457–481, 1958 (anchors §V-A's right-censored survival reporting); [28] Liberzon, *Switching in Systems and Control*, Birkhäuser 2003 (anchors the §III-B statement of what a dwell-time constraint would be — the honest completion of the deleted false Liberzon claim). Cross-check after additions: 28/28 cited, no orphans; pdf-verify and sweep exit 0.

✅ **b8 REPLACED (author-directed, 2026-08-18).** The unverifiable entry ("A Review on Swarm Resilience Enhancement", claimed ICRA 2025 — title unindexed anywhere, venue-less PDF) is replaced with an indexed peer-reviewed review supporting the same single claim ("decentralized swarms offer a resilient approach [8], [23]"): **A. Hamid, Y. Almoghathawi, A. Alghazi, H. Saleh, "Enhancing resilience in UAV swarms: A literature review," *Journal of Safety Science and Resilience*, 2025** (journal identified by ISSN 2666-4496 matching the article PII; authors/title/year web-verified; volume omitted rather than guessed). The venue-less PDF stays in `refrences/` as reading material but is no longer cited.

✅ **b14 was a CHIMERA — fixed.** The old entry grafted the arXiv preprint's title and author list (Shetty, **Knowles**, Gao, "…Using Distributed ADMM-based Trajectory Planning" — repo file `2012.09808v3.pdf`) onto the journal venue. The actually-published NAVIGATION 70(1) 2023 paper is **Shetty, Hussain, Gao, "Decentralized Connectivity Maintenance for Multi-Robot Systems Under Motion and Sensing Uncertainties," Art. no. navi.552** (verified from navi.ion.org). Entry corrected; the §II-C in-text usage still fits. This was the last never-verified non-canonical entry; every bibliography item is now either verified against its source PDF/publisher page or is a canonical textbook citation (b2, b3, b5, b7).
...
### Em-dash removal and title-named PDF (author-requested, 2026-08-18)

All 44 authored em dashes (`---`; no literal U+2014 existed) replaced with context-appropriate punctuation: parentheses for asides, commas for appositives, colons/semicolons for elaborations. Each substitution asserted to match exactly once; post-condition `'---' count == 0` asserted before writing. (First attempt aborted safely on the known heredoc `\`-collapse pitfall, caught by the per-item assert with nothing written; re-run from a file.) Numeric-range en dashes (`--`, e.g. `1000--1049`) intentionally untouched. One U+2014 remains in the compiled text layer: IEEEtran's own `Abstract—` heading separator, generated by the class file, not authored text; left as the template mandates. Compiled (tectonic, exit 0, 12 pages); `verify_manuscript_pdf.py` CLEAN exit 0; title-named copy written with PDF metadata Title/Author set; sweep allowlist extended for the title-named PDF (compressed payload, same justification as `final_manuscript.pdf`) and the private Q&A docx (withdrawal-context quotes); sweep CLEAN exit 0.
...
- [x] **`python experiments/sweep_retired_claims.py` → exit 0** (final run 2026-08-18, unpiped true exit code). **`python experiments/verify_manuscript_pdf.py` → exit 0** likewise. Encoding-agnostic (UTF-8/16LE/16BE/32LE, zip members opened, digit-boundary-aware). Exists because the UTF-16 `mc_results` files defeated every plain grep this project ran; the tool's docstring states exactly what it cannot reach (compressed PDF/PNG payloads, text rendered into images, git history) so the guarantee has known limits rather than assumed totality. Must be re-run at submission time, not just once.
- [x] **PDF recompiled and VERIFIED** (2026-08-18). Compiled with tectonic 0.15.0 → `manuscript/final_manuscript.pdf` (12 pages, 5 embedded figures); stale `draft 11.pdf` deleted. Compiled output checked by `experiments/verify_manuscript_pdf.py` (now a gate tool): corrected numbers present (127±5/128±5, 0.261±0.017/0.0499, 50/50 censored, 5.2×, 1.55×, +1.81, 0.0159±0.0013), retired set absent outside the two withdrawal narratives, placeholders VISIBLY present (ORCID string, funding-agency thanks), all 4 authors render, provenance + eviction subsections present. → `PDF VERIFICATION CLEAN`, exit 0.
  Verifier lessons kept in-file: hyphenation ("cen-sored") and a too-narrow withdrawal-context window produced two false FAILs on first run — verifier artifacts, fixed in the checker, not the paper. Separately, the byte-level sweep "hit" `1999` inside the PDF's deflate stream — a codec coincidence; compressed payloads are excluded from byte-scanning WITH justification, and the text-layer verifier is authoritative for this artifact. One process bug caught: piping the sweep through `tail` reported `tail`'s exit code as the sweep's — one earlier "CLEAN" line in the working log was based on the wrong exit status (the underlying runs were in fact clean, re-verified unpiped).
- [x] **ORCID — RESOLVED by author decision (2026-08-18):** the 4th author was removed from the paper, and the placeholder ORCID belonged to her; both are verified ABSENT from the compiled PDF (`verify_manuscript_pdf.py`, exit 0).
- [x] **Author list — RESOLVED by author decision (2026-08-18):** 3 authors (Malge, Hegde, Koushik K R); PDF and `.tex` now agree, README citation updated, verifier asserts exactly these three render.
